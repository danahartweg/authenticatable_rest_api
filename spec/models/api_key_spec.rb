...
describe ApiKey do
	...
	it "has a valid factory" do
		create(:api_key).should be_valid
	end
	...
	it "generates an access token" do
		create(:api_key).access_token.should =~ /\S{32}/
	end

	it "must accept web or iOS as a scope" do
		create(:api_key).scope.should be_in %w( web iOS )
	end

	it "shouldn't accept scopes other than web or iOS" do
		build(:api_key, scope: 'badScope').should_not be_valid
	end

	it "sets expiration properly for the web scope" do
		api_key = build(:api_key, scope: 'web')
	...
	it "sets expiration properly for the iOS scope" do
		api_key = build(:api_key, scope: 'iOS')
		api_key.set_expiry_date
	...
	it "is associated with a user" do
		api_key = create(:api_key)
		api_key.user = create(:user)

		api_key.user.should be_valid
	end

	it "can be expired" do
		api_key = build(:api_key, expires_at: 60.days.from_now)
